...
X4 <- data.frame(
    x1 = rnorm(500, mean = 10, sd = 1),
    x2 = rnorm(500, mean = -5, sd = 1),
    class = "red"
)
X5 <- data.frame(
    x1 = rnorm(600, mean = 4, sd = 1),
    x2 = rnorm(600, mean = 8, sd = 1),
    class = "white"
)
...
lda2 <- function(data) {
    ...
    runifx1 <- runif(100000, min = 1.2 * min(minx[, 1]), max = 1.2 * max(maxx[, 1]))
    runifx2 <- runif(100000, min = 1.2 * min(minx[, 2]), max = 1.2 * max(maxx[, 2]))
    ...
    for (i in (1:n_cla)) {
        ...
        hulls[[i]] <- concaveman(points[[i]], concavity = 10e20)
        lines(hulls[[i]])
    }
